"""Prometheus metrics for event-shortener.
...
keep cardinality bounded (the public `/{ident}` redirect would otherwise blow up
the label set with one series per short link).
"""
...
URLS_CREATED_TOTAL = Counter(
    ...
    "Short URLs created (new idents minted; idempotent re-calls are not counted).",
)
REDIRECTS_TOTAL = Counter(
    "shortener_redirects_total",
    "Public redirect lookups by result.",
    ["result"],  # ok | expired | not_found
)
