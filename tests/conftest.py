"""Real-seam test harness for event-shortener.

The service talks to PostgreSQL through ``SqlExecutor`` running raw SQL, and the
behaviours under test (idempotency, the ident UNIQUE constraint + collision
retry, the timestamptz redirect window) are all DB semantics — so the suite
runs against a real PostgreSQL, not a fake.
...
API_KEY = "test-shortener-key"

os.environ.setdefault("SHORTENER_API_KEY", API_KEY)
...
class _EphemeralPostgres:
    ...
    def __init__(self) -> None:
        self.tmpdir = Path(tempfile.mkdtemp(prefix="pg-shortener-test-"))
    ...
    def start(self) -> str:
        ...
        subprocess.run(
            ["createdb", "-h", "127.0.0.1", "-p", str(self.port), "-U", "postgres", "event_shortener"],
            check=True,
            capture_output=True,
        )
        return f"postgresql+asyncpg://postgres@127.0.0.1:{self.port}/event_shortener"
...
async def _clean_db(_migrated: str) -> AsyncGenerator[None]:
    """Truncate the table before each test so cases start from an empty schema."""
    from sqlalchemy import text
    from sqlalchemy.ext.asyncio import create_async_engine

    eng = create_async_engine(_migrated)
    async with eng.begin() as conn:
        await conn.execute(text("TRUNCATE short_urls RESTART IDENTITY"))
    await eng.dispose()
    return
